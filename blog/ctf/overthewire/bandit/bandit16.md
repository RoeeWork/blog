---
layout: post
title: "bandit16"
---

20-07-2026
18:54
![]({{ '/ctf/overthewire/bandit/pics/Pasted%20image%2020260720190438.png' | relative_url }})

ill do a service scan on that range using nmap:
![]({{ '/ctf/overthewire/bandit/pics/Pasted%20image%2020260720192407.png' | relative_url }})
probably the ssl/unkown service, so our port is 31790:
![]({{ '/ctf/overthewire/bandit/pics/Pasted%20image%2020260720192700.png' | relative_url }})
got keyupdate? 
using -ign_eof:
![]({{ '/ctf/overthewire/bandit/pics/Pasted%20image%2020260720193227.png' | relative_url }})

key:
```
-----BEGIN OPENSSH PRIVATE KEY-----

-----END OPENSSH PRIVATE KEY-----
```
